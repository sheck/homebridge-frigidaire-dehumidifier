...
const DEHUMIDIFIERMODES = new Set([
  DEHMODE_DRY,
  DEHMODE_AUTO,
  DEHMODE_CONTINUOUS,
  DEHMODE_QUIET,
...
class Frigidaire extends EventEmitter {
  ...
  constructor(log, config) {
    ...
    this.isBusy = false
    this.sessionKeyRefreshTime =
      (config.sessionKeyRefresh ?? FRIGIDAIRE_SESSIONKEY_TIMEOUT) * 3600000
  }
    ...
    try {
      ...
      if (authResponse) {
        ...
        if (this.sessionKeyRefreshTime > 0) {
          this.log.info(
            `Frigidaire Session Key will be refresh in ${Math.floor(
              (this.sessionKeyRefreshTime / (1000 * 60 * 60)) % 24
            )} hour(s) and ${Math.floor(
              (this.sessionKeyRefreshTime / (1000 * 60)) % 60
            )} min(s).`
          )
          this.sessionKeyRefreshHandle = setTimeout(
            () => this.refreshSessionKey(),
            this.sessionKeyRefreshTime
          )
        } else
          this.log.info(
            `Automatic Frigidaire Session Key refresh has been disabled.`
          )
      } else return false
  ...
  async authenticate() {
    ...
    try {
      ...
      this.log.error(
        "Frigidaire Login Error: ",
        err.response.body.code + " " + err.response.body.message
      )
      this.isBusy = false
      return false
    }
  ...
  async isValidSession() {
    ...
    if (!sessionValidationResponse || sessionValidationResponse == "")
      return false
    return true
  }
  ...
  async getDetailForDevice(deviceIndex) {
    ...
    var urlDeviceString =
      "?pnc=" +
      this.frig_devices[deviceIndex].pnc +
      "&elc=" +
      this.frig_devices[deviceIndex].elc +
      "&sn=" +
      this.frig_devices[deviceIndex].serialNumber +
      "&mac=" +
      this.frig_devices[deviceIndex].mac +
    ...
    try {
      ...
      for (var i = 0; i < detailJSON.length; i++) {
        switch (detailJSON[i]["haclCode"]) {
          case HUMIDITY_ROOM:
            this.frig_devices[deviceIndex].roomHumidity =
              detailJSON[i]["numberValue"]
            break
          case MODE:
          ...
          case CLEAN_AIR_MODE:
            ...
            break
          case CHILD_MODE:
            this.frig_devices[deviceIndex].childMode =
              detailJSON[i]["numberValue"]
          ...
          case FIRMWARE_VERSION:
            this.frig_devices[deviceIndex].firmwareVersion =
              detailJSON[i]["stringValue"]
    ...
    if (
      hasMonitoredValuesChanged !=
      this.frig_devices[deviceIndex].monitoredValues
    ) {
  ...
  async getRequest(endpoint) {
    ...
    try {
      const response = await superagent.get(url).set(headers).disableTLSCerts()
      if (response.body.status == "ERROR" && response.body.code != "ECP0000") {
        this.log.error(
          "Frigidaire Get Error: " +
            response.body.code +
            " " +
            response.body.message
        )
  ...
  async setDevicePowerMode(deviceIndex, onValue = false) {
    ...
    if (onValue)
      returnCode = await this.sendDeviceCommand(
        deviceIndex,
        POWER_MODE,
        POWER_ON
      )
  ...
  async setDehumidifierRelativeHumidity(deviceIndex, humidityLevel = 50) {
    ...
    this.log.error(
      "Dehumidifier Humidity Level not within acceptable range. Value must be between 35 and 85.",
      err
    )
    return -1
  }
  ...
  async sendDeviceCommand(deviceIndex, attribute, value) {
    ...
    if (!(await this.isValidSession())) {
      this.log.warn(
        "Send command detect login no longer valid. Attempting to re-login"
      )
  ...
  startPollingProcess() {
    ...
    this.deviceRefreshHandle = setTimeout(
      () => this.backgroundRefresh(),
      this.deviceRefreshTime
    )
  }
  ...
  async backgroundRefresh() {
    ...
    if (this.isBusy) {
      this.log.warn(
        "Another process is already updating. Skipping Interval Update."
      )
      this.deviceRefreshHandle = setTimeout(
        () => this.backgroundRefresh(),
        this.deviceRefreshTime
    ...
    if (!(await this.isValidSession())) {
      // session is expired or not login, get new session key
      this.log.warn(
        "Background refreshing detected sessions is no longer valid, attempting to re-login."
      )
