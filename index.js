...
class FrigidaireAppliancePlatform {
  constructor(log, config, api) {
    ...
    try {
      if (
        ...
        this.log.error(
          "Plug-in configuration error: Frigidaire Application authentication information not provided."
        )
  ...
  async refreshAccessories() {
    ...
    for (var i = 0; i < this.frig.frig_devices.length; i++) {
      ...
      if (currentDevice.destination == DEHUMIDIFIER) {
        this.log(
          `Configuring ${currentDevice.name} with a Device ID: ${currentDevice.deviceId}`
        )
        let deviceAccessory = new dehumidifierAppliance(
          this.frig,
          i,
          currentDevice,
          this.config,
        ...
        let foundAccessory = this.accessories.find(
          (accessory) => accessory.UUID === deviceAccessory.uuid
        )
        if (!foundAccessory) {
          // create a new accessory
          let newAccessory = new this.api.platformAccessory(
            deviceAccessory.name,
            deviceAccessory.uuid
          )
          ...
          // accessory already exist just set characteristic
        ...
        if (this.enableAirPurifier) {
          let deviceAccessoryAir = new airpurifierAppliance(
            this.frig,
            i,
            currentDevice,
            this.config,
          ...
          // check the accessory was not restored from cache
          let foundAccessory = this.accessories.find(
            (accessory) => accessory.UUID === deviceAccessoryAir.uuid
          )
          if (!foundAccessory) {
            // create a new accessory
            let newAccessory = new this.api.platformAccessory(
              deviceAccessoryAir.name,
              deviceAccessoryAir.uuid
            )
            // add services and Characteristic
    ...
    if (homekit_appliance_count != 0) {
      this.log.info(
        `Frigidaire background update process started. Appliance status will be check each ${Math.floor(
          this.config.deviceRefresh / 60
        )} min(s) ${Math.floor(this.config.deviceRefresh % 60)} second(s).`
      )
  ...
  async orphanAccessory() {
    ...
    for (var i = 0; i < cachedAccessory.length; i++) {
      ...
      foundAccessory = this.frig.frig_devices.find(
        (device) =>
          UUIDGen.generate(device.deviceId.toString()) === accessory.UUID
      )
      if (!foundAccessory) {
        if (this.enableAirPurifier) {
          ...
          foundAccessory = this.frig.frig_devices.find(
            (device) =>
              UUIDGen.generate(
                device.deviceId.toString() + "-" + CLEAN_AIR_MODE
              ) === accessory.UUID
          )
          if (!foundAccessory) this.removeAccessory(accessory, true)
        } else this.removeAccessory(accessory, true)
      }
    }
  }
